...
class TemplatesModelStyle extends JModelAdmin
{
    ...
    public function save($data)
    {
        ...
        if ($user->authorise('core.edit', 'com_menus') && $table->client_id == 0) {
            ...
            $query->where('template_style_id = ' . (int)$table->id)->where('checked_out IN (0,' . (int)$user->id . ')');
            $db->setQuery($query);
            $db->execute();
    ...
    public function setHome($id = 0)
    {
        ...
        $db->setQuery('UPDATE #__template_styles' . ' SET home = \'1\'' . ' WHERE id = ' . (int)$id);
        $db->execute();
    ...
    public function unsetHome($id = 0)
    {
        ...
        $db->setQuery('SELECT client_id, home' . ' FROM #__template_styles' . ' WHERE id = ' . (int)$id);
        $style = $db->loadObject();
        ...
        // Set the new home style.
        $db->setQuery('UPDATE #__template_styles' . ' SET home = \'0\'' . ' WHERE id = ' . (int)$id);
        $db->execute();
