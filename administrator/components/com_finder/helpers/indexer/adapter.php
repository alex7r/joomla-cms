...
abstract class FinderIndexerAdapter extends JPlugin
{
    ...
    protected function getContentCount()
    {
        ...
        if ($query instanceof JDatabaseQuery) {
            $query = clone $query;
            $query->clear('select')->select('COUNT(*)')->clear('order');
        }
    ...
    protected function getUpdateQueryByTime($time)
    {
        ...
        $query = $this->db->getQuery(true)->where('a.modified >= ' . $this->db->quote($time));
    ...
    protected function getUpdateQueryByIds($ids)
    {
        ...
        $query = $this->db->getQuery(true)->where('a.id IN(' . implode(',', $ids) . ')');
