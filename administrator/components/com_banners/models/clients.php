...
class BannersModelClients extends JModelList
{
    ...
    public function getItems()
    {
        ...
        $query->clear('where')->where('state = 0')->where('cid IN (' . $clientIds . ')');
        $db->setQuery($query);
        ...
        $query->clear('where')->where('state = -2')->where('cid IN (' . $clientIds . ')');
        $db->setQuery($query);
    ...
    protected function getListQuery()
    {
        ...
        $query->select('COUNT(b.id) as nbanners')->join('LEFT', '#__banners AS b ON a.id = b.cid');
