...
if ($parameters->get('redirect_edit', 'site') == 'site') {
    $editUrl = JUri::base(
        ) . 'index.php?option=com_config&controller=config.display.modules&id=' . (int)$mod->id . $redirectUri;
    $target  = '_self';
}
...
$moduleHtml = preg_replace(// Replace first tag of module with a class
    ...
    '\\1 jmoddiv" data-jmodediturl="' . $editUrl . '" data-target="' . $target . '" data-jmodtip="' . JHtml::tooltipText(
        ...
        htmlspecialchars(
            ...
            JText::_('JLIB_HTML_EDIT_MODULE_IN_POSITION'),
    ...
    . ($menusEditing && $mod->module == 'mod_menu' ? '" data-jmenuedittip="' . JHtml::tooltipText(
