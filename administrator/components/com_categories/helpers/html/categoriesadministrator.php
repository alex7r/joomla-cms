...
abstract class JHtmlCategoriesAdministrator
{
    ...
    public static function association($catid, $extension = 'com_content')
    {
        ...
        if ($associations = CategoriesHelper::getAssociations($catid, $extension)) {
            ...
            $db    = JFactory::getDbo();
            $query = $db->getQuery(true)->select('c.id, c.title')->select('l.sef as lang_sef')->from(
                    '#__categories as c'
                )->where('c.id IN (' . implode(',', array_values($associations)) . ')')->join(
                    'LEFT',
                    '#__languages as l ON c.language=l.lang_code'
                )->select('l.image')->select('l.title as language_title');
            $db->setQuery($query);
            ...
            if ($items) {
                foreach ($items as &$item) {
                    ...
                    $url          = JRoute::_(
                        'index.php?option=com_categories&task=category.edit&id=' . (int)$item->id . '&extension=' . $extension
                    );
                    $tooltipParts = array(
                        JHtml::_(
                            'image',
