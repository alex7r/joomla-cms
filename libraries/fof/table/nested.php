...
class FOFTableNested extends FOFTable
{
    ...
    public function getParent()
    {
        ...
        if (empty($this->treeParent) || !is_object(
            ...
            $table = $this->getClone();
            $table->reset();
            $this->treeParent = $table->whereRaw($fldLft . ' = ' . $db->q($targetLft))->get()->current();
        }
    ...
    public function insertAsRoot()
    {
        ...
        $query  = $db->getQuery(true)->select('MAX(' . $fldRgt . ')')->from($db->qn($this->getTableName()));
    ...
    public function getRoot()
    {
        ...
        if (empty($this->treeRoot) || !is_object($this->treeRoot) || !($this->treeRoot instanceof FOFTableNested)) {
            ...
            $subQuery = $db->getQuery(true)->select('MIN(' . $fldLft . ')')->from($db->qn($this->getTableName()));

            try {
                $table = $this->getClone();
                $table->reset();
                $root = $table->whereRaw($fldLft . ' = (' . (string)$subQuery . ')')->get(0, 1)->current();
