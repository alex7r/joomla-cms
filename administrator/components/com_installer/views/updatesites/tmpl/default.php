...
<div id="installer-manage" class="clearfix">
	<form action="<?php echo JRoute::_('index.php?option=com_installer&view=updatesites'); ?>" method="post"
				...
				<?php else : ?>
					<table class="table table-striped">
						...
						<?php foreach ($this->items as $i => $item) : ?>
							...
							if ($item->enabled == 2) echo ' protected'; ?>">
								<td class="center">
								...
								<span class="small break-word">
									<a href="<?php echo $item->location; ?>"
									   target="_blank"><?php echo $this->escape($item->location); ?></a>
								</span>
									</label>
								</td>
								<td class="hidden-phone">
							<span class="bold hasTooltip"
							      title="<?php echo JHtml::tooltipText($item->name, $item->description, 0); ?>">
								<?php echo $item->name; ?>
							</span>
								</td>
