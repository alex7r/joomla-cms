...
class HttpClient
{
	...
	{
		if (empty($this->_url) || empty($this->_httpMethod))
		{
		...
		else if ($this->_httpMethod == HttpClient::DELETE)
		{
			curl_setopt($curl, CURLOPT_CUSTOMREQUEST, 'DELETE');
		}
		else if ($this->_httpMethod == HttpClient::GET)
		{ /*NO ACTION NECESSARY*/
		}
		...
		if ($this->_trace)
		{
			...
			echo "PARAMETTERS: ";
			if (is_array($this->_jsonParams))
			{
				echo print_r($this->_jsonParams);
			}
			else echo "NONE";
			{
				echo "\n";
