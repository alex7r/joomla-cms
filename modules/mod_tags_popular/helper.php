...
abstract class ModTagsPopularHelper
{
    ...
    public static function getList(&$params)
    {
        ...
        $query = $db->getQuery(true)->select(
            ...
            )->group($db->quoteName(array('tag_id', 'title', 'access', 'alias')))->from(
        ...
        if ($language != 'all') {
            ...
            $query->where(
                $db->quoteName('t.language') . ' IN (' . $db->quote($language) . ', ' . $db->quote('*') . ')'
            );
        }
        ...
        $query->join('INNER', $db->quoteName('#__tags', 't') . ' ON ' . $db->quoteName('tag_id') . ' = t.id')->join(
                'INNER',
                $db->qn(
                    '#__ucm_content',
                    'c'
                ) . ' ON ' . $db->qn('m.core_content_id') . ' = ' . $db->qn('c.core_content_id')
            );

        $query->where($db->quoteName('m.type_alias') . ' = ' . $db->quoteName('c.core_type_alias'));

        // Only return tags connected to published articles
        $query->where($db->quoteName('c.core_state') . ' = 1')->where(
                '(' . $db->quoteName('c.core_publish_up') . ' = ' . $nullDate . ' OR ' . $db->quoteName(
                    'c.core_publish_up'
                ) . ' <= ' . $db->quote($nowDate) . ')'
            )->where(
                '(' . $db->quoteName('c.core_publish_down') . ' = ' . $nullDate . ' OR  ' . $db->quoteName(
                    'c.core_publish_down'
                ) . ' >= ' . $db->quote($nowDate) . ')'
            );
        ...
        if ($order_value == 'rand()') {
            ...
            if ($params->get('order_value', 'title') == 'title') {
                ...
                $equery = $db->getQuery(true)->select(
                    array(
                        'a.tag_id',
                        'a.count',
                        'a.title',
